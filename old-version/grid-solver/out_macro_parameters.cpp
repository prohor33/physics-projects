...
void OutMacroParameters::PrintPercent(clock_t start, int main_circle){
	// надо не забыть стартануть время вначале
	int last_procent = -1;
	int procent;
	double workTime;
	...
	if( last_procent != procent ){
		...
		if( procent == 0 ) {
			printf("Time passed: %.0fs\tTime left: can't tell yet", (int)workTime);
			return;
		}
  ...
  for(int y=0; y<Grid_->size_y; y++) {
    for(int x=0; x<Grid_->size_x; x++) {
			if( !Cells[x][y] || Cells[x][y]->type == 9 ) {
				...
				mod_gas_velocity = std::sqrt( q_x*q_x + q_y*q_y );
  ...
  for(int y=0; y<Grid_->size_y; y++) {
    for(int x=0; x<Grid_->size_x; x++) {
			if( !Cells[x][y] || Cells[x][y]->type == 9 ) {
					...
					for(int j=0; j<MainInfo_->speed_quantity; j++) {
						for(int k=0; k<MainInfo_->speed_quantity; k++) {
							if( Cells[x][y]->GetP(i,j,k) > MainInfo_->p_cut &&
								...
								// вроде без масс скорость должна быть, а то это уже импульс какой-то
								// или расход массы
								// почему то  у второго! газа и скорость и,
								// соответсвенно, расход газа в несколько раз(может и в 10) болше первого!!!
								// а, нене, все норм
								// так и должно быть распределение по скоростям такое
								// у газа с большей массой просто больше скорости
								// хотя это и странно!
								u_x += Cells[x][y]->real_SPEED_CUBE[i][j][k]*
				...
				u_x /= n;
				u_y /= n;
				mod_gas_velocity = std::sqrt( u_x*u_x + u_y*u_y );
				if( mod_gas_velocity > max_mod_gas_velocity )
					max_mod_gas_velocity = mod_gas_velocity;
				if( mod_gas_velocity < min_mod_gas_velocity )
					min_mod_gas_velocity = mod_gas_velocity;
...
bool OutMacroParameters::FillConsuptionAreas( void ) {
	// нужно найти эти самые интересующие нас места в сетке (области подсчета расхода газа)
	...
	for( int i = 0; i<main_info->gaps_quantity; i++ ) {
		...
		consuptions_areas[2*i]->name = "gap_" + to_string((long double)i) + "_down";
		// область (сверху)
		...
		consuptions_areas[2*i+1]->name = "gap_" + to_string((long double)i) + "_up";
	}
	cons_areas_quantity = main_info->gaps_quantity*2;
	// теперь надо разобраться с входами и выходами каналов
